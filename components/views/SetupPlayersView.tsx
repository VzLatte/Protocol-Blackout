...
export const SetupPlayersView: React.FC<SetupPlayersViewProps> = ({ game }) => {
  ...
  return (
    <ScreenWrapper visualLevel={game.visualLevel} centerContent={false}>
      ...
      <div className="flex-1 p-6 flex flex-col items-center justify-start pb-20 pt-10">
         <div className="w-full max-w-2xl bg-slate-900/60 backdrop-blur-xl border border-slate-800 p-8 rounded-[3rem] shadow-2xl animate-in fade-in duration-500">
            <h2 className="text-xl font-black uppercase text-white italic mb-8 border-b border-slate-800 pb-4 flex items-center gap-3">
               ...
               <div className="space-y-4">
                  ...
                  <div className="space-y-3 max-h-[50vh] overflow-y-auto pr-2 custom-scrollbar">
                      {playerConfigs.slice(0, game.setupCount).map((cfg: any, i: number) => (
                         <div key={i} className="bg-black/40 border border-slate-800 p-5 rounded-[2rem] space-y-4 transition-all hover:border-slate-700">
                            <div className="flex justify-between items-center">
                               <div className="flex items-center gap-3 flex-1">
                                  {game.editingNameIdx === i ? (
                                    <div className="flex gap-2 w-full animate-in slide-in-from-left duration-200">
                                       <input autoFocus value={game.tempName} onChange={(e) => game.setTempName(e.target.value)} className="flex-1 bg-teal-500/10 border border-teal-500 p-2 rounded-xl text-xs text-teal-400 outline-none font-bold uppercase" />
                                       <button onClick={game.saveEditName} className="p-2 bg-teal-500 text-black rounded-xl"><Check size={16} /></button>
                                       <button onClick={game.cancelEditName} className="p-2 bg-red-500 text-white rounded-xl"><X size={16} /></button>
                                    </div>
                                  ) : (
                                    <>
                                       <span className="text-sm font-black italic uppercase text-white truncate max-w-[120px]">{cfg.name}</span>
                                       <button onClick={() => game.startEditName(i)} className="text-slate-600 hover:text-teal-500 p-1"><Edit2 size={12} /></button>
                                    </>
                                  )}
                               </div>
                               <div className="flex bg-slate-800/80 p-1 rounded-xl">
                                  <button onClick={() => updatePlayerConfig(i, { isAI: false })} className={`flex items-center gap-2 px-3 py-1.5 rounded-lg text-[10px] font-bold uppercase transition-all ${!cfg.isAI ? 'bg-teal-500 text-black' : 'text-slate-500'}`}><User size={12}/> Human</button>
                                  <button onClick={() => updatePlayerConfig(i, { isAI: true })} className={`flex items-center gap-2 px-3 py-1.5 rounded-lg text-[10px] font-bold uppercase transition-all ${cfg.isAI ? 'bg-amber-500 text-black shadow-[0_0_15px_rgba(245,158,11,0.2)]' : 'text-slate-500'}`}><Cpu size={12}/> AI</button>
                               </div>
                            </div>
                                     ...
                                     <div className="flex flex-wrap gap-1">
                                        {[AIArchetype.TURTLE, AIArchetype.AGGRO, AIArchetype.STRATEGIST].map(arch => (
                                           <button 
                                              ...
                                              className={`px-3 py-1 rounded-lg text-[8px] font-black uppercase border transition-all ${cfg.archetype === arch ? 'bg-amber-500/10 border-amber-500 text-amber-500' : 'bg-transparent border-slate-800 text-slate-600'}`}
                                           >
                                              {arch}
                                           </button>
                                        ))}
