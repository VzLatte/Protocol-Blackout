...
import { Phase, VisualLevel, HazardType, DifficultyLevel } from '../../shared/types';
import { Button } from '../ui/Button';
import { Lock, ChevronLeft, Zap, Flame, Radio, AlertTriangle, Cpu, Target, Info, ShieldAlert } from 'lucide-react';
import { Modal } from '../ui/Modal';
import { CAMPAIGN_LEVELS } from '../../shared/campaignRegistry';

interface NodeSelectorViewProps {
  game: any;
  // now pass the full level object to avoid downstream lookup errors
  onSelectLevel: (level: any) => Promise<void> | void;
  onBack: () => void;
}
          ...
          <div className="space-y-8 pb-4">
             ...
             <Button 
               ...
               onClick={async () => {
                  playSfx('startup');
                  try {
                     // pass the whole level object (safer than passing only id)
                     await onSelectLevel(selectedLvl);
                     setSelectedLvl(null);
                  } catch (err) {
                     // don't let an exception during navigation break the entire UI
                     console.error('Failed to initialize level:', err);
                     playSfx('error');
                  }
               }}
             >
