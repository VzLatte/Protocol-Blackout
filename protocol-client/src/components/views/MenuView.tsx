...
import { GameMode, Phase, VisualLevel } from '../../shared/types';
...
interface MenuViewProps {
  ...
  // SYSTEM ACTIONS (Passed from useGameState)
  onHelp: () => void;
  onSettings: () => void;
  onExitRequest: () => void; // Map this to game.setIsExitConfirming(true)
...
export const MenuView: React.FC<MenuViewProps> = ({ 
  ...
  return (
    <ScreenWrapper visualLevel={visualLevel} centerContent={false}>
      {/* FIX: GlobalHeader now gets the explicit handlers. 
         'onExit' here triggers the 'Are you sure?' modal.
      */}
      <GlobalHeader 
        phase={Phase.MENU} 
        onHelp={onHelp} 
      ...
      <div className="flex-1 w-full flex flex-col items-center justify-center p-6 pt-12 pb-24 space-y-16 text-center animate-in fade-in slide-in-from-bottom-4 duration-700">
         ...
         <div className="pt-8">
           <Button variant="ghost" size="lg" onClick={onBack} className="px-12 py-4">
              <ChevronLeft size={16} className="mr-2" /> Return to Type Selection
           </Button>
         </div>
      </div>
    </ScreenWrapper>
