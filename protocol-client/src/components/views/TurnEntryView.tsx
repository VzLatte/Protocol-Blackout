...
import { Shield, Skull, ChevronUp, Brain, Zap, Activity, Heart, Move, CornerUpLeft, Trash2, Target, Ban, AlertTriangle, Minus, Plus, Clock } from 'lucide-react';
import WaitingForOpponent from '../ui/WaitingForOpponent';
import { BASE_ATTACK_DMG, ATTACK_CONFIG, BLOCK_CONFIG, BASE_MOVE_COST } from '../../../src/shared/constants';
import { Phase, ActionType, UnitType, Position, TileType, DamageType } from '../../../src/shared/types';
...
import { calculateIncome, ROLLOVER_CAP } from '../../../src/shared/apManager';
...
export const TurnEntryView: React.FC<TurnEntryViewProps> = ({ game }) => {
  const { playSfx, credits, submitAction, tutorial, advanceTutorial, distanceMatrix, isMultiplayerMode, timeLimit } = game;
  const p = game.players[game.currentPlayerIdx];
  const otherPlayers = game.players.filter((x: any) => x.id !== p.id && !x.isEliminated);

  // Multiplayer submit state & timers
  const isMultiplayer = !!isMultiplayerMode;
  const [localSubmitted, setLocalSubmitted] = useState(false);
  const [turnSecondsLeft, setTurnSecondsLeft] = useState<number>(isMultiplayer ? 15 : (game.turnTimer || 30));
  ...
  const touchStartY = useRef(0);
   ...
   if (!p || !p.loadout) {
    return (
      <ScreenWrapper visualLevel={game.visualLevel} centerContent={true}>
        <div className="flex flex-col items-center gap-4">
          <Activity className="text-teal-500 animate-spin" size={48} />
          <div className="text-mono text-teal-500 uppercase tracking-widest animate-pulse">
  ...
  // Countdown for multiplayer turn entry (15s)
  useEffect(() => {
    if (!isMultiplayer) return;
    if (p?.hasSubmitted || localSubmitted) return; // stop if already submitted

    setTurnSecondsLeft(15);
    const id = window.setInterval(() => {
      setTurnSecondsLeft(s => {
        if (s <= 1) {
          // Auto-submit a default action if player hasn't submitted
          submitAction({ blockAp: 0, attackAp: 0, moveAp: 0, abilityActive: false });
  ...
  }, [isMultiplayer, p?.hasSubmitted, localSubmitted, submitAction]);

  // Reset localSubmitted when server reflects submission or when phase changes
  useEffect(() => {
    if (!isMultiplayer) return;
    if (p?.hasSubmitted) setLocalSubmitted(false);
  }, [isMultiplayer, p?.hasSubmitted]);
  ...
  return (
    <ScreenWrapper visualLevel={game.visualLevel} className="bg-[#020617]" noScroll centerContent={false}>
      ...
      <div className="flex-1 w-full bg-[#050b14] relative overflow-hidden flex flex-col items-center pt-4">
          ...
          {isMultiplayer && (p?.hasSubmitted || localSubmitted) && (
            <WaitingForOpponent secondsLeft={game.timeLimit || turnSecondsLeft} onCancel={() => { setLocalSubmitted(false); }} />
          )}
      </div>
         ...
         >
            ...
            <div className="grid grid-cols-2 gap-4">
                ...
                <div className="flex flex-col items-end">
                   ...
                   {isMultiplayer && !(p?.hasSubmitted || localSubmitted) && (
                     <div className="mt-2 text-[12px] font-mono text-slate-400 flex items-center gap-2">
                       <Clock size={12} /> <span>{timeLimit ?? turnSecondsLeft}s</span>
                     </div>
                   )}
                </div>
            </div>
         ...
         <div className="absolute bottom-0 left-0 right-0 p-4 bg-gradient-to-t from-[#0a0f1e] via-[#0a0f1e] to-transparent z-30">
            <Button 
               ...
               onClick={() => {
                 ...
                 // Immediately show waiting screen until server reflects submission
                 setLocalSubmitted(true);
               }}
               disabled={remainingAp < 0 || (moveMode && localMovePath.length === 0) || p?.hasSubmitted || localSubmitted}
            >
               {remainingAp < 0 ? 'OVER_BUDGET' : moveMode && localMovePath.length === 0 ? 'PLOT_COURSE' : (p?.hasSubmitted || localSubmitted) ? 'SUBMITTED' : 'EXECUTE'}
            </Button>
         </div>
      </div>
    </ScreenWrapper>
