...
import { DifficultyLevel, HazardType, Phase, GameMode, Player, AIDifficulty, AIArchetype, WinCondition, UnitType } from '../../src/shared/types';
...
export function useCampaignManager(
  ...
  // Accept either a level id string OR the full level object to be permissive
  const startCampaignLevel = async (levelOrId: string | any) => {
    const lvl = typeof levelOrId === 'string' ? CAMPAIGN_LEVELS.find(l => l.id === levelOrId) : levelOrId;
    if (!lvl) return;

    battle.setMode(GameMode.TACTICAL);
    battle.setMaxRounds(15); // Standard Campaign limit
    setCurrentCampaignLevelId(lvl.id);
    ...
    const playerUnit = progression.unlockedUnits && progression.unlockedUnits.length > 0 ? progression.unlockedUnits[0] : UnitType.PYRUS;

    const newPlayers: Player[] = [{ 
      name: 'OPERATIVE', id: 'p0', isAI: false, hp: INITIAL_HP, maxHp: INITIAL_HP, ap: baseAp, unit: UNITS[playerUnit], isEliminated: false, moveFatigue: 0, blockFatigue: 0, statuses: [], cooldown: 0, activeUsed: false, desperationUsed: false, totalReservedAp: 0, isAbilityActive: false,
      position: { x: 0, y: 5 }, 
      captureTurns: 0,
