...
import { Player, TurnData, ResolutionLog, ActionType, UnitType, GameMode, DifficultyLevel, HazardType, TileType, GridMap, Position, DamageType, ItemSlot } from '../shared/types';
...
import { calculateIncome } from '../shared/apManager';
import { getManhattanDistance, getObstaclesInLine, getCoverStatus, getStride, isValidPos } from '../shared/utils/gridLogic';
...
export function resolveCombat(
  ...
  let frame = 0; // Visual frame index for grouping simultaneous logs
  const pushLog = (l: any) => { l.frame = frame; logs.push(l); };
  // Monkey-patch direct logs.push calls so they also receive the frame index
  const _origPush = logs.push.bind(logs);
  (logs as any).push = (...items: any[]) => {
    items.forEach(it => {
      it.frame = frame;
      // Populate attacker/target names when available for clearer logs
      if (it.attackerId && !it.attackerName) {
        const p = nextPlayers.find(n => n.id === it.attackerId);
        if (p) it.attackerName = p.name;
      }
      if (it.targetId && !it.targetName) {
        const t = nextPlayers.find(n => n.id === it.targetId);
        if (t) it.targetName = t.name;
      }
      _origPush(it);
    });
    return logs.length;
          ...
          if (!p.desperationUsed && p.unit) {
              ...
              switch (p.unit.type) {
                  case UnitType.GHOST:
                      ...
                      pushLog({ attackerId: p.id, type: ActionType.DESPERATION, resultMessage: "PHANTOM STEP: PHASED & HEALED" });
                      break;
                  case UnitType.AEGIS:
  ...
  // Advance visual frame: end of Desperation
  frame++;
  ...
  submissions.forEach(sub => {
    ...
    if (!sub.action.isDesperation) {
        ...
        const prevBlockFatigue = p.blockFatigue || 0;
        if (sub.action.blockAp > 0) p.blockFatigue += 1;
        else {
            if (prevBlockFatigue > 0) pushLog({ attackerId: p.id, type: ActionType.VENT, resultMessage: 'VENT: COOLING COMPLETE' });
            p.blockFatigue = 0;
        }
        p.moveFatigue = 0;
  ...
  // Advance visual frame: end of Preparation
  frame++;
  ...
  // Advance visual frame after all attacks so simultaneous attacks are grouped together
  frame++;
